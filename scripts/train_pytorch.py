...
class LossIntervalTracker:
    """
    log_interval ごとに loss の平均を計算して保存する。
    さらに、平均推移を図で保存できる。
    ...
    def save(self, *, dpi: int = 160) -> Path:
        """
        図で保存する。色指定はしない（デフォルトカラー）。
        """
        ...
        # loss
        plt.plot(x, self.loss_mean)
        plt.title(f"Teacher Loss Curves (interval mean)")
        plt.xlabel("global_step")
        plt.ylabel("loss")
        plt.grid(True)

        plt.tight_layout()
        plt.savefig(self.out_path, dpi=dpi)
        plt.close()
        return self.out_path

def make_loss_plot_path(config, exp_checkpoint_dir: Path) -> Path:
    return "teacher"/ exp_checkpoint_dir / "loss_curves.png"
